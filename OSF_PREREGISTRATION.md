...
```mermaid
flowchart TB
    A[ OSF Preregistration<br/>Регистрация OSF] --> B[ Primary Hypothesis<br/>Основная Гипотеза]
    A --> C[ Sampling Plan<br/>План Выборки]
    A --> D[ Analysis Plan<br/>План Анализа]
    A --> E[ Variables / Переменные]
    
    B --> B1["H1: Hyperbolic field exposure<br/>produces measurable changes"]
    C --> C1["5-10 donors / 5-10 доноров"]
    C --> C2["30-60 plasma samples"]
    D --> D1["ANOVA with post-hoc"]
    D --> D2["p < .05 significance"]
    E --> E1["Clot formation time"]
    E --> E2["Clot degradation rate"]
